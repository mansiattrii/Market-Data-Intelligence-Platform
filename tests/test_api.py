...
def test_metric_time_series_unknown_metric():
    response = client.get("/companies/AAPL/metrics/not_a_real_metric")
    assert response.status_code == 404


def test_metric_time_series_unknown_ticker():
    response = client.get("/companies/NOTATICKER/metrics/revenue")
    assert response.status_code == 404


def test_metric_time_series_bad_date_range():
    response = client.get(
...
def test_top_metric_unknown_metric():
